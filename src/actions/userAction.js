...
export const register = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `https://timekeeper-back-end.herokuapp.com/customers`,
      userData,
      config,
    )
...
export const getUserDetails = (id) => async (dispatch) => {
  try {
    ...
    dispatch({ type: USER_DETAILS_SUCCESS, payload: data.customer })
  } catch (error) {
...
export const updateUser = (id, userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`http://localhost:5000/customers/${id}`, userData, config)
...
export const deleteUser = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:5000/customers/${id}`)
